...
const REFERENCE_DATE = new Date(2025, 7, 9);
...
function attachGlobalListeners() {
  ...
  document.body.addEventListener('change', (e) => {
    ...
    if (checkboxSelectors.some(s => e.target.matches(s))) {
...
function initPerson(container, personId, isSupp = false) {
  ...
  if (suppProductsContainer) suppProductsContainer.innerHTML = generateSupplementaryProductsHtml(personId);
  ...
  if (sclSection) {
    ...
    const handleProgramChange = () => {
      const chosen = programSelect.value !== '';
      outpatientCheckbox.disabled = !chosen;
      dentalCheckbox.disabled = !chosen;
    ...
    const handleMainCheckboxChange = () => {
      const checked = mainCheckbox.checked && !mainCheckbox.disabled;
      programSelect.disabled = !checked;
      scopeSelect.disabled = !checked;
      sclSection.querySelector('.product-options').classList.toggle('hidden', !checked);
      if (checked) {
  ...
  ['bhn', 'accident', 'hospital-support'].forEach(product => {
    ...
    checkbox.addEventListener('change', () => {
      section.querySelector('.product-options').classList.toggle('hidden', !(checkbox.checked && !checkbox.disabled));
      calculateAll();
    });
  });
...
function initSupplementaryButton() {
  document.getElementById('add-supp-insured-btn').addEventListener('click', () => {
    ...
    el.className = 'person-container space-y-6 bg-gray-100 p-4 rounded-lg mt-4';
    el.id = `person-container-${personId}`;
    el.innerHTML = generateSupplementaryPersonHtml(personId, supplementaryInsuredCount);
    container.appendChild(el);
    initPerson(el, personId, true);
    updateSupplementaryAddButtonState();
    calculateAll();
  });
}
...
function renderWaiver30Section(mainPersonInfo) {
  ...
  if (eligibleSupps.length > 0) {
    radios += `<div class="space-y-2">`;
    eligibleSupps.forEach(p => {
      radios += `<label class="flex items-center space-x-3 cursor-pointer">
  ...
  radios += `<label class="flex items-center space-x-3 cursor-pointer mt-2">
    ...
    <div id="waiver30-other-form" class="${prev === 'other' ? '' : 'hidden'} mt-3">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select class="form-select waiver-other-gender"><option value="Nam">Nam</option><option value="Nữ">Nữ</option></select>
        </div>
        <div class="relative">
...
function getWaiverOtherInfo() {
  ...
  if (/^\d{2}\/\d{2}\/\d{4}$/.test(v)) {
    ...
    if (!isNaN(d) && d <= REFERENCE_DATE) {
      age = REFERENCE_DATE.getFullYear() - d.getFullYear();
      const m = REFERENCE_DATE.getMonth() - d.getMonth();
      if (m < 0 || (m === 0 && REFERENCE_DATE.getDate() < d.getDate())) age--;
    }
  }
...
function calculateWaiver30Premium(mainPersonInfo, mainPremiumDisplay) {
  ...
  const mainProduct = document.getElementById('main-product')?.value || '';
  if (mainProduct === 'TRON_TAM_AN') { if (feeEl) feeEl.textContent = ''; return 0; }

  const selected = cont.querySelector('input[name="waiver30-insurer"]:checked')?.value || '';
  if (!selected) { if (feeEl) feeEl.textContent = ''; return 0; }
  ...
  // tổng phí bổ sung của tất cả NĐBH bổ sung; riêng người được chọn (nếu là trong danh sách) sẽ trừ
  let sumSuppAllSupp = 0;
  let suppOfSelected = 0;
  document.querySelectorAll('#supplementary-insured-container .person-container').forEach(p => {
    const pInfo = getCustomerInfo(p, false);
    const container = p.querySelector('.supplementary-products-container');
    if (!container) return;
    let sub = 0;
    sub += calculateHealthSclPremium(pInfo, container);
    sub += calculateBhnPremium(pInfo, container);
    sub += calculateAccidentPremium(pInfo, container);
    const baseMainPremium = mainPremiumDisplay - getExtraPremiumValue();
    sub += calculateHospitalSupportPremium(pInfo, baseMainPremium, container, 0);
    sumSuppAllSupp += sub;
  ...
  // tuổi/giới tính người chọn
  let insuredAge = 0;
  let insuredGender = 'Nam';
  if (selected === 'other') {
    if (!validateWaiverOtherForm()) { if (feeEl) feeEl.textContent = ''; return 0; }
    const other = getWaiverOtherInfo();
    insuredAge = other.age;
    insuredGender = other.gender;
  } else {
    const pEl = document.getElementById(selected);
    const info = pEl ? getCustomerInfo(pEl, false) : null;
    if (!info || info.age < WAIVER30.MIN_AGE || info.age > WAIVER30.MAX_AGE) { if (feeEl) feeEl.textContent = ''; return 0; }
    insuredAge = info.age; insuredGender = info.gender;
  }

  const rateRow = product_data.waiver30_rates?.find(r => insuredAge >= r.ageMin && insuredAge <= r.ageMax);
  if (!rateRow) { if (feeEl) feeEl.textContent = ''; return 0; }
...
function initSummaryModal() {
  ...
  document.getElementById('main-product').addEventListener('change', () => {
    updateTargetAge();
    if (modal.classList.contains('hidden')) calculateAll(); else generateSummaryTable();
  });
...
/* Formatters & Occupation autocomplete */
...
function calculateAll() {
  try {
    ...
    // render Waiver sau khi đã có phí cơ bản và bổ sung
    renderWaiver30Section(mainPersonInfo);

    let waiverPremium = 0;
    try { waiverPremium = calculateWaiver30Premium(mainPersonInfo, mainPremiumDisplay); } catch (e) { waiverPremium = 0; }
      ...
      if (sectionId === 'health-scl') {
        ...
        if (mainProduct === 'TRON_TAM_AN') {
          ...
          programSelect.querySelectorAll('option').forEach(opt => {
            if (opt.value === '') return;
            if (mainProduct === 'TRON_TAM_AN') opt.disabled = false;
            else if (mainPremium >= 15000000) opt.disabled = false;
...
function renderMainProductOptions(customer) {
  ...
  if (mainProduct === 'TRON_TAM_AN') {
    ...
    let options = '';
    if (age <= 55) options += '<option value="15">15 năm</option>';
    if (age <= 60) options += '<option value="10">10 năm</option>';
    if (age <= 65) options += '<option value="5">5 năm</option>';
    if (!options) options = '<option value="" disabled>Không có kỳ hạn phù hợp (tuổi vượt quá 65)</option>';
    html += `
      <div><label class="font-medium text-gray-700 block mb-1">Thời hạn đóng phí</label>
      <select id="abuv-term" class="form-select">${options}</select>
  ...
  if (['KHOE_BINH_AN', 'VUNG_TUONG_LAI', 'PUL_TRON_DOI', 'PUL_15_NAM', 'PUL_5_NAM'].includes(mainProduct)) setPaymentTermHint(mainProduct, age);
}
...
function calculateMainPremium(customer, ageOverride = null) {
  ...
  if (mainProduct.startsWith('PUL') || mainProduct === 'AN_BINH_UU_VIET' || mainProduct === 'TRON_TAM_AN')) {
    ...
    if (mainProduct.startsWith('PUL')) {
      ...
      if (!ageOverride) {
        if (stbh > 0 && stbh < 100_000_000) setFieldError(stbhEl, 'STBH nhỏ hơn 100 triệu'); else clearFieldError(stbhEl);
        if (premium > 0 && premium < 5_000_000) setFieldError(stbhEl, 'Phí chính nhỏ hơn 5 triệu');
      }
    } else if (mainProduct === 'AN_BINH_UU_VIET') {
    ...
    if (!ageOverride && rangeEl) rangeEl.textContent = `Phí hợp lệ từ ${formatCurrency(minFee, '')} đến ${formatCurrency(maxFee, '')}.`;
    const entered = parseFormattedNumber(document.getElementById('main-premium-input')?.value || '0');
    if (!ageOverride) {
      const feeInput = document.getElementById('main-premium-input');
      if (stbh > 0 && entered > 0) {
        const invalid = (entered < minFee || entered > maxFee || entered < 5_000_000);
...
function calculateHealthSclPremium(customer, container, ageOverride = null) {
  ...
  let fee = 0;
  fee += product_data.health_scl_rates[scope]?.[idx]?.[program] || 0;
  if (hasOutpatient) fee += product_data.health_scl_rates.outpatient?.[idx]?.[program] || 0;
  if (hasDental) fee += product_data.health_scl_rates.dental?.[idx]?.[program] || 0;

  if (!ageOverride) section.querySelector('.fee-display').textContent = fee > 0 ? `Phí: ${formatCurrency(fee)}` : '';
  return fee;
}
function calculateBhnPremium(customer, container, ageOverride = null) {
  ...
  if (stbh < 100_000_000 || stbh > MAX_STBH.bhn) { setFieldError(stbhInput, 'STBH không hợp lệ, từ 100 triệu đến 5 tỷ'); throw new Error('STBH không hợp lệ, từ 100 triệu đến 5 tỷ'); } else clearFieldError(stbhInput);
...
function calculateAccidentPremium(customer, container, ageOverride = null) {
  ...
  const stbh = parseFormattedNumber(stbhInput?.value || '0');
  if (stbh === 0) { if (!ageOverride) section.querySelector('.fee-display').textContent = ''; return 0; }
  if (stbh < 100_000_000 || stbh > MAX_STBH.accident) { setFieldError(stbhInput, 'STBH không hợp lệ, từ 100 triệu đến 8 tỷ'); throw new Error('STBH không hợp lệ, từ 100 triệu đến 8 tỷ'); } else clearFieldError(stbhInput);
...
function calculateHospitalSupportPremium(customer, mainPremium, container, totalHospitalSupportStbh = 0, ageOverride = null) {
  ...
  const totalMaxSupport = Math.floor(mainPremium / 4_000_000) * 100_000;
  const maxByAge = ageToUse >= 18 ? 1_000_000 : 300_000;
  const remaining = totalMaxSupport - totalHospitalSupportStbh;

  if (!ageOverride) section.querySelector('.hospital-support-validation').textContent =
    `Tối đa: ${formatCurrency(Math.min(maxByAge, remaining), 'đ/ngày')}. Phải là bội số của 100.000.`;
  ...
  if (stbh % 100_000 !== 0) { setFieldError(hsInput, 'STBH không hợp lệ, phải là bội số 100.000'); throw new Error('STBH không hợp lệ, phải là bội số 100.000'); }
  if (stbh > maxByAge || stbh > remaining) { setFieldError(hsInput, 'Vượt quá giới hạn cho phép'); throw new Error('Vượt quá giới hạn cho phép'); }
...
function updateSummaryUI(premiums) {
  ...
  suppContainer.innerHTML = premiums.totalSupplementaryPremium > 0
    ? `<div class="flex justify-between items-center py-2 border-b"><span class="text-gray-600">Tổng phí SP bổ sung:</span><span class="font-bold text-gray-900">${formatCurrency(premiums.totalSupplementaryPremium)}</span></div>`
    : '';
...
function generateSummaryTable() {
  ...
  try {
    ...
    if (mainProduct === 'TRON_TAM_AN') {
      ...
      if (!healthSclCheckbox?.checked || healthSclPremium === 0) throw new Error('Trọn Tâm An bắt buộc phải kèm Sức Khỏe Bùng Gia Lực.');
    }
    ...
    if (['PUL_TRON_DOI', 'PUL_5_NAM', 'PUL_15_NAM', 'KHOE_BINH_AN', 'VUNG_TUONG_LAI'].includes(mainProduct)
      ...
      throw new Error(`Độ tuổi mục tiêu phải ≥ ${mainPersonInfo.age + paymentTerm - 1} đối với ${mainProduct}.`);
    }

    const suppPersons = [];
    document.querySelectorAll('.person-container').forEach(p => { if (p.id !== 'main-person-container') suppPersons.push(getCustomerInfo(p, false)); });
    ...
    const totalMaxSupport = Math.floor(initialBaseMainPremium / 4_000_000) * 100_000;
    ...
    if (waiverSelected && waiverPerYear > 0) {
      if (waiverSelected === 'other') waiverStartAge = otherInfo?.age || 0;
      else {
    ...
    for (let i = 0; (mainPersonInfo.age + i) <= targetAge; i++) {
      ...
      if (waiverPerYear > 0 && waiverStartAge > 0 && (waiverStartAge + i) <= WAIVER30.MAX_RENEWAL_AGE) waiverThisYear = waiverPerYear;
      ...
      const suppPremiums = suppPersons.map(person => {
        ...
        let sp = 0;
        if (suppContainer) {
          sp += calculateHealthSclPremium({ ...person, age: currentPersonAge }, suppContainer, currentPersonAge);
          sp += calculateBhnPremium({ ...person, age: currentPersonAge }, suppContainer, currentPersonAge);
          sp += calculateAccidentPremium({ ...person, age: currentPersonAge }, suppContainer, currentPersonAge);
          sp += calculateHospitalSupportPremium({ ...person, age: currentPersonAge }, initialBaseMainPremium, suppContainer, totalHospitalSupportStbh, currentPersonAge);
          const hs = parseFormattedNumber(suppContainer.querySelector('.hospital-support-stbh')?.value || '0');
          if (suppContainer.querySelector('.hospital-support-checkbox')?.checked && hs > 0) totalHospitalSupportStbh += hs;
        }
        if (waiverSelected === person.container.id && waiverThisYear > 0) sp += waiverThisYear;
        totalSuppAccAll += sp;
        return sp;
      });
      ...
      if (waiverSelected === 'other') { html += `<td class="p-2 border text-right">${formatCurrency(waiverThisYear)}</td>`; totalWaiverAcc += waiverThisYear; }
      const rowTotal = mainPremiumForYear + suppPremiumMain + suppPremiums.reduce((s, v) => s + v, 0) + (waiverSelected === 'other' ? waiverThisYear : 0);
    ...
    if (waiverSelected === 'other') html += `<td class="p-2 border text-right">${formatCurrency(totalWaiverAcc)}</td>`;
    const grand = totalMainAcc + totalSuppAccMain + totalSuppAccAll + (waiverSelected === 'other' ? totalWaiverAcc : 0);
    html += `<td class="p-2 border text-right">${formatCurrency(grand)}</td></tr></tbody></table>`;
...
function exportToHTML(mainPersonInfo, suppPersons, targetAge, initialMainPremiumWithExtra, paymentTerm) {
  // Rút gọn: tái sử dụng summary đã hiển thị là đủ. Có thể mở rộng nếu cần.
  window.print();
}
...
function setFieldError(inputOrEl, message) {
  ...
  if (message) input.classList.add('border-red-500'); else input.classList.remove('border-red-500');
}
function clearFieldError(inputOrEl) { setFieldError(inputOrEl, ''); }
function validateMainPersonInputs() {
  const container = document.getElementById('main-person-container'); if (!container) return true;
  const nameInput = container.querySelector('.name-input');
  const dobInput = container.querySelector('.dob-input');
  const occupationInput = container.querySelector('.occupation-input');
  let ok = true;
  ...
  if (dobInput) {
    const v = (dobInput.value || '').trim(); const re = /^\d{2}\/\d{2}\/\d{4}$/;
    if (!re.test(v)) { setFieldError(dobInput, 'Ngày sinh không hợp lệ, nhập DD/MM/YYYY'); ok = false; }
    else {
      const [dd, mm, yyyy] = v.split('/').map(n => parseInt(n,10));
      const d = new Date(yyyy, mm-1, dd);
  ...
  if (occupationInput) {
    ...
    if (!match || !(group >= 1 && group <= 4)) { setFieldError(occupationInput, 'Chọn nghề nghiệp từ danh sách'); ok = false; } else clearFieldError(occupationInput);
  }
  return ok;
}
...
function getPaymentTermBoundsByProduct(mainProduct, age) {
  const min = getProductMinPaymentTerm(mainProduct);
...
function validateSection2FieldsPreCalc(customer) {
  ...
  if (['KHOE_BINH_AN','VUNG_TUONG_LAI','PUL_TRON_DOI','PUL_15_NAM','PUL_5_NAM'].includes(mainProduct)) {
    ...
    if (el) {
      ...
      if (el.value && (isNaN(val) || val < min || val > max)) setFieldError(el, `Thời hạn không hợp lệ, từ ${min} đến ${max}`); else clearFieldError(el);
    }
  }
  if (['KHOE_BINH_AN','VUNG_TUONG_LAI'].includes(mainProduct)) {
    const stbh = parseFormattedNumber(document.getElementById('main-stbh')?.value || '0');
    const feeInput = document.getElementById('main-premium-input');
    const row = product_data.mul_factors.find(f => customer.age >= f.ageMin && customer.age <= f.ageMax);
    if (row && stbh > 0) {
      const minFee = stbh / row.maxFactor, maxFee = stbh / row.minFactor;
      const rangeEl = document.getElementById('mul-fee-range');
      ...
      if (entered > 0 && (entered < minFee || entered > maxFee || entered < 5_000_000)) setFieldError(feeInput, 'Phí không hợp lệ'); else clearFieldError(feeInput);
    }
  }
}
...
function getHealthSclStbhByProgram(program) {
  switch (program) { case 'co_ban': return 100_000_000; case 'nang_cao': return 250_000_000; case 'toan_dien': return 500_000_000; case 'hoan_hao': return 1_000_000_000; default: return 0; }
}
...
function generateSupplementaryProductsHtml() {
  return `
      ...
      <div class="product-options hidden mt-3 pl-8 space-y-4 border-l-2 border-gray-200">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div><label class="font-medium text-gray-700 block mb-1">Phạm vi địa lý</label>
            <select class="form-select health-scl-scope" disabled><option value="main_vn">Việt Nam</option><option value="main_global">Nước ngoài</option></select>
          </div>
        </div>
    ...
    <div class="product-section bhn-section hidden">
      <label class="flex items-center space-x-3 cursor-pointer"><input type="checkbox" class="form-checkbox bhn-checkbox"><span class="text-lg font-medium text-gray-800">Bảo hiểm Bệnh Hiểm Nghèo 2.0</span></label>
